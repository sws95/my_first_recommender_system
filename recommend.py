...
class RandomRecommender(BaseRecommender):
    def recommend(self, dataset: Dataset, **kwargs) -> RecommendResult:
        ...
        # 사용자-아이템 행렬에서 각 셀의 예측 평점값은 0.5~5.0의 균등 난수로 한다.
        pred_matrix = np.random.uniform(
            0.5, 5.0, (len(unique_user_ids), len(unique_movie_ids))
        )

        # RMSE 평가용으로 테스트 데이터에 나오는 사용자와 아이템의 예측 평점값을 저장한다.
        ...
        for i, row in dataset.test.iterrows():
            ...
            # 테스트 데이터에 나타나는 사용자 ID와 아이템 ID의 인덱스를 얻어
            # 평점값 행렬값을 취득한다.
        ...
        # 순위 평가용 데이터 작성
        # 각 사용자에 대한 추천 영화는
        # 해당 사용자가 아직 평가하지 않은 영화 중에서 무작위 10개 작품으로 한다.
